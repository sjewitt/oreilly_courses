...
@pytest.mark.skip(reason="we know how to add up already")
def test_add_2():
    assert add(10,20) == 40 # !

def test_multiply():
    assert multiply(10,20) == 200
# argh!! he uses the python version conition again!! which doesn't fucKiNg work!!!
# FFS!! The OS version vondition WORKS here! WTF??
@pytest.mark.skipif(sys.version_info > (3,11),reason="TOOO OLD")
def test_divide():
